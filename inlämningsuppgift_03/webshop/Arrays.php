
<?php

$names = array(
    0 => "Daniels andra vrålåk",
    1 => "Lynchburg-lemonade",
    2 => "Nattuggla",
    3 => "Frangelico",
    4 => "Mugg",
    5 => "DVD film Golfaren",
    6 => "Georg",
    7 => "BMW 320",
    8 => "VW passat",
    9 => "AWP - Dragon-Lore"
);

$descriptions = array(
    0 => "Penisförlängare",  
    1 => "En whiskey drink med cointreu",
    2 => "En riktigt jävla jobbig uggla som spelar boogabooo",   
    3 => "Mycket söt likör med tydlig smak av hasselnötter! Passar utmärkt efter en kopp kaffe!",
    4 => "Fin mugg som är lite kantstött",
    5 => "Filmen golfaren, skapad av Lasse Åberg",
    6 => "Duplo figur av Greta gris lillebror Georg gris",
    7 => "Daniels andra BMW",
    8 => "Marcus PASSATjävel",
    9 => "Ett skin i spelet CSGO som kostar uppemot 15000kr i finaste kvalite"
);

$images = array(
    0 => "http://localhost/TestAPI/bilder/bil.jpg",  
    1 => "http://localhost/TestAPI/bilder/lynchburg-lemonade2.jpg",
    2 => "http://localhost/TestAPI/bilder/sovuggla.jpg",   
    3 => "http://localhost/TestAPI/bilder/frangelico_bottle.jpg",
    4 => "http://localhost/TestAPI/bilder/mugg.jpg",
    5 => "http://localhost/TestAPI/bilder/Driver.jpg",
    6 => "http://localhost/TestAPI/bilder/georg.jpg",
    7 => "http://localhost/TestAPI/bilder/bil2.jpg",
    8 => "http://localhost/TestAPI/bilder/passatjävel.jpg",
    9 => "http://localhost/TestAPI/bilder/dragon-lore.jpg"
);


$prices = array(
    0 => "56 000",
    1 => "79",
    2 => "Paraply på köpet",
    3 => "139",
    4 => "10",
    5 => "20",
    6 => "35",
    7 => "65 000",
    8 => "10 000 kr och stänkskydd till din cykel (vilket du kommer behöva)",
    9 => "7500"
);

